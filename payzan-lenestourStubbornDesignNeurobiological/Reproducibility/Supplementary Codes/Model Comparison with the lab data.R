...
#Fit the CbD Model with no uncertainty, and with value distortion instead of probability distortion----
Param_CbD_v=matrix(0,N_ID,7)#beta_logit, alpha, lambda, Kappa1, kappa2, theta, w, for each participant
OOS_CbD_v=rep(0,N_ID)#OOS goodness-of-fit for each agent
Likelihood_CbD=function(Param_agent,Stats_agent,Train){#Stats_agent is the supercut matrix from Sequence_bank for that agent, with the 7th and 8th columns replaced if needed
  beta_logit=Param_agent[1]
  alpha=Param_agent[2]
  lambda=Param_agent[3]
  ...
  for(t in 1:N_trial){
    ...
    DA=max(0,1/(1+exp(-1*kappa1*G))-kappa2)
    V=uncertainty*(prob_win*(reward-0.7)^alpha-lambda*(1-prob_win)*0.7^alpha)
    Bet_prob_agent[t]=logit(V+w*DA,beta_logit)
    if(Stats_agent[t,7] != 2){#Exclude the missing trials
      if(Stats_agent[t,7] == 1){#bet
        logl[t]=max(log(Bet_prob_agent[t]),-50)
      } else{#pass
...
for(ID in Index){
  ...
  range_high_CbD=c(60,1,2.5,0.4,1,1,3)
  Stats_agent=Sequence_bank[ID,,]
  Stats_agent[,7]=Choice_fit[ID,]
  Stats_agent[,8]=outcome_fit[ID,]
  Stats_agent_train=Stats_agent[Train_trials,]
  Stats_agent_test=Stats_agent[Test_trials,]
  if(Bet_yellow_fit[ID]>0){
    initial_CbD=c(40,1,1,0.2,0.7,0.95,1)
  } else{
    initial_CbD=c(40,1,1,0,0.7,0.95,1)
  }
  result=optim_wrapper(initial_CbD,Likelihood_CbD,Stats_agent=Stats_agent_train, method="L-BFGS-B",lower=range_low_CbD,upper=range_high_CbD)
  # Check if optimization was successful
  if (!is.null(result)) {
    Param_CbD_v[ID,]<- result$par
    OOS_CbD_v[ID]=Likelihood_CbD(Param_CbD_v[ID,],Stats_agent_test)
  } else {
    Param_CbD_v[ID,] <- NA  # Set to NA or handle as needed
    OOS_CbD_v[ID,] <- NA
    Error_seed=c(Error_seed,i)
    print(i)
  }
...
#Fit the RL model with initial Q as free values and undirectional exploration-----
Param_RL_explore=matrix(0,N_ID,6)#beta_logit, eta, the 3 Q values for the 2 training sequences, and undirectional exploration
OOS_RL_explore=rep(0,N_ID)#OOS goodness-of-fit for each agent
Likelihood_RL=function(Param_agent,Stats_agent){#Stats_agent is the supercut matrix from Sequence_bank for that agent, with the 7th and 8th columns replaced if needed
  beta_logit=Param_agent[1]
  eta=Param_agent[2]
  Q_initial=Param_agent[3:5]
...
for(ID in 1:N_ID){
  range_low_RL=c(40,0.1,rep(-0.7,3),0)
  range_high_RL=c(60,1,rep(1.3,3),1)
  Stats_agent=Sequence_bank[ID,,]
  Stats_agent[,7]=Choice_fit[ID,]
  Stats_agent[,8]=outcome_fit[ID,]
  Stats_agent_train=Stats_agent[Train_trials,]
  Stats_agent_test=Stats_agent[Test_trials,]
  Param_RL_explore[ID,]=optim(c(40,0.5,rep(0,3),0),Likelihood_RL,Stats_agent=Stats_agent_train, method="L-BFGS-B",lower=range_low_RL,upper=range_high_RL)$par
  Param_RL_test_explore=c(Param_RL_explore[ID,1:2],runif(3,-0.7,1.3),Param_RL_explore[ID,6])
  OOS_RL_explore[ID]=Likelihood_RL(Param_RL_explore[ID,],Stats_agent_test)
}

#Fit the RL model with initial Q as free values and context dependence-----
Param_context_RL=matrix(0,N_ID,8)#beta_logit, eta, the 6 Q values for the 2 training sequences
OOS_context_RL=rep(0,N_ID)#OOS goodness-of-fit for each agent
Likelihood_RL=function(Param_agent,Stats_agent){#Stats_agent is the supercut matrix from Sequence_bank for that agent, with the 7th and 8th columns replaced if needed
  beta_logit=Param_agent[1]
  eta=Param_agent[2]
  Q_initial=Param_agent[3:8]
  N_trial=dim(Stats_agent)[1]
  Bet_prob_agent=rep(0,N_trial)
  logl=rep(0,N_trial)
  #Q_RL=runif(1,-0.7,1.3)
  for(t in 1:N_trial){
    ...
    if(Stats_agent[t,3]!=Stats_agent[t-1,3] || t==1){#reset the Q-value for a new sequence
      seq=seq+1
      Q_RL=Q_initial[(seq*2+1):(seq*2+2)]
      Q_skip=0
    }
    prob_win=Stats_agent[t,4]
    uncertainty=Stats_agent[t,5]
    reward=Stats_agent[t,6]
    k=ifelse(prob_win==0.8,1,2)
    Bet_prob_agent[t]=exp(beta_logit*Q_RL[k])/(exp(beta_logit*Q_RL[k])+exp(beta_logit*Q_skip))
    if(Stats_agent[t,7] != 2){#Exclude the missing trials
      if(Stats_agent[t,7] == 1){#bet
        logl[t]=max(log(Bet_prob_agent[t]),-30)
        Q_RL[k]=(1-eta)*Q_RL[k]+eta*Stats_agent[t,8]
      } else{#pass
        logl[t]=max(log(1-Bet_prob_agent[t]),-30)
      }
    }
...
for(ID in 1:N_ID){
  range_low_context_RL=c(40,0.1,rep(-0.7,6))
  range_high_context_RL=c(60,1,rep(1.3,6))
  Stats_agent=Sequence_bank[ID,,]
  Stats_agent[,7]=Choice_fit[ID,]
  Stats_agent[,8]=outcome_fit[ID,]
  Stats_agent_train=Stats_agent[Train_trials,]
  Stats_agent_test=Stats_agent[Test_trials,]
  Param_context_RL[ID,]=optim(c(40,0.5,rep(0,6)),Likelihood_RL,Stats_agent=Stats_agent_train, method="L-BFGS-B",lower=range_low_context_RL,upper=range_high_context_RL)$par
  OOS_context_RL[ID]=Likelihood_RL(Param_context_RL[ID,],Stats_agent_test)
}
``



#save.image("Model comparison with the lab data_beta40to60_exploration and loss tolerance.RData")
