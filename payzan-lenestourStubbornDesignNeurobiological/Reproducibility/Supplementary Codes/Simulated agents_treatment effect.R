...
#Simulate the choices of CbD agents with no uncertainty effect, and with value distortion instead of probability distortion-----
Choice_CbD_v=matrix(0,198,630)
outcome_CbD_v=matrix(0,198,630)
Bet_prob_CbD_v=matrix(0,198,630)
entropy_high=-1*(0.1*log(0.1)+0.4*log(0.4)+0.5*log(0.5))
entropy_low=-1*(0.2*log(0.2)+0.8*log(0.8))
control=1:99
test=100:198
...
for(ID in 1:N_ID){
  ...
  kappa2=rtruncnorm(1, a=0.7, b=0.9, mean = 0.8, sd = 0.05)
  theta=rtruncnorm(1, a=0.9, b=1, mean = 0.95, sd = 0.05)
  w=rtruncnorm(1, a=1, b=1.1, mean = 1, sd = 0.02)
  for(t in 1:N_trial){
    if(t>2 && Sequence_bank[ID,t,3]>Sequence_bank[ID,t-1,3]) {G=0}#Reset the influence in each sequence
    prob_win=Sequence_bank[ID,t,4]
    uncertainty=Sequence_bank[ID,t,5]
    ...
    DA=max(0,1/(1+exp(-1*kappa1*G))-kappa2)
    V=uncertainty*(prob_win*(reward-0.7)^alpha-lambda*(1-prob_win)*0.7^alpha)
    Bet_prob_CbD_v[ID,t]=logit(V+w*DA)
    Choice_CbD_v[ID,t]=sample(c(0, 1), size = 1, prob = c(1-Bet_prob_CbD_v[ID,t], Bet_prob_CbD_v[ID,t]))
    if(Choice_CbD_v[ID,t]==1){#The corresponding outcome
      if(uncertainty==1){
        outcome_CbD_v[ID,t]=sample(c(0, reward), size = 1, prob = c(1-prob_win, prob_win))-0.7
      } else{
        outcome_CbD_v[ID,t]=sample(c(0, 0.7, reward), size = 1, prob = c((1-prob_win)/2, 0.5, prob_win/2))-0.7
      }
    }
    G=G*theta+I(outcome_CbD_v[ID,t]>0)
  }
}

c(mean(Choice_CbD_v[control,][control_blue_index]), 
  mean(Choice_CbD_v[test,][test_blue_index]),
  mean(Choice_CbD_v[control,][control_yellow_index]),
  mean(Choice_CbD_v[test,][test_yellow_index]))
...
table_3_extend=matrix(0,9,4)
rownames(table_3_extend)=c("RL (Type 1)", "RL (Type 2)", "RL (Type 2)+ UE", "RL (context)", "Base (RN)", "Pavlovian-augmented (RN)", "Base (LT)", "Pavlovian-augmented (LT)","Lab Data")
colnames(table_3_extend)=c("Blue, Control", "Blue, Test", "Yellow, Control", "Yellow, Test")
table_3_extend[c(1,2,5,6,9),]=table_3[1:5,]
...
table_3_extend[3,]=c(mean(Choice_RL_2[control,][control_blue_index]), 
              ...
              mean(Choice_RL_2[control,][control_yellow_index]),
              mean(Choice_RL_2[test,][test_yellow_index]))
#Simulate the choices of context RL agents------
beta_logit=30
Choice_context_RL=matrix(0,198,630)
Bet_prob_context_RL=matrix(0,198,630)
eta=0.1
for(ID in 1:N_ID){
  sequence=1
  #Q_RL=runif(1,-0.7,1.3)
  for(t in 1:N_trial){
    if(sequence<ceiling(t/105) || t==1){#reset the Q-value for a new sequence
      Q_RL=c(0.5,-0.05)#the initial Q value for blue and yellow sessions, respectively
      #Q_RL=runif(1,-0.7,1.3)#The Q value of bet, the Q-value of skip is always 0
      sequence=ceiling(t/105)
      Q_skip=0
    }
    ...
    Bet_prob_context_RL[ID,t]=exp(beta_logit*Q_RL[k])/(exp(beta_logit*Q_RL[k])+exp(beta_logit*Q_skip))
    Choice_context_RL[ID,t]=sample(c(0, 1), size = 1, prob = c(1-Bet_prob_context_RL[ID,t], Bet_prob_context_RL[ID,t]))
    if(Choice_context_RL[ID,t]==1){#The corresponding outcome
      if(uncertainty==1){
        outcome=sample(c(0, reward), size = 1, prob = c(1-prob_win, prob_win))-0.7
      } else{
        outcome=sample(c(0, 0.7, reward), size = 1, prob = c((1-prob_win)/2, 0.5, prob_win/2))-0.7
      }
      Q_RL[k]=(1-eta)*Q_RL[k]+eta*outcome
      Q_skip=Q_skip
    } else{#skip
      Q_skip=(1-eta)*Q_skip
      Q_RL[k]=Q_RL[k]
...
table_3_extend[4,]=c(mean(Choice_context_RL[control,][control_blue_index]), 
  mean(Choice_context_RL[test,][test_blue_index]),
  mean(Choice_context_RL[control,][control_yellow_index]),
  mean(Choice_context_RL[test,][test_yellow_index]))
...
table_3_extend[7,]=c(mean(Choice_rational[control,][control_blue_index]), 
                     mean(Choice_rational[test,][test_blue_index]),
                     mean(Choice_rational[control,][control_yellow_index]),
                     mean(Choice_rational[test,][test_yellow_index]))

table_3_extend[8,]=c(mean(Choice_CbD[control,][control_blue_index]), 
                     mean(Choice_CbD[test,][test_blue_index]),
                     mean(Choice_CbD[control,][control_yellow_index]),
                     mean(Choice_CbD[test,][test_yellow_index]))
