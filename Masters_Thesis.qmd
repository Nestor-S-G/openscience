...
``` r
> ATDs1s2.summary<-data.frame(treatment=levels(ATDs1s2$Variable),
mean=tapply(ATDs1s2$value,ATDs1s2$Variable,mean),n=tapply(ATDs1s2$value,ATDs1s2$Variable,length),sd=tapply(ATDs1s2$value,ATDs1s2$Variable,sd))
Error in data.frame(treatment = levels(ATDs1s2$Variable), mean = tapply(ATDs1s2$value,  : 
...
Therefore, it cannot be reproduced with only the two conditions indicated in the python_requirements.txt file. I proceed to install xlrd and modify the line:

``` python
---> 17     temp = pd.read_excel(x, sheet_name = 'results', header = None)
```

With:

``` python
---> 17 temp = pd.read_excel(x, sheet_name = 'results', header = None, engine = ‘xlrd’)
```
...
``` bash
FATAL: While performing build: conveyor failed to get: GET https://index.docker.io/v2/nipreps/mriqc/manifests/0.15.1: MANIFEST_UNKNOWN: manifest unknown; unknown tag=0.15.1
```
...
Which produces a new error. I check that the repository still exists by accessing [https://github.com/OpenNeuroDatasets/ds003745]{.underline}
...
I check the structure of the *bids* folder at [https://bids-standard.github.io/bids-validator/]{.underline} and this yields several errors, including:
...
R version 4.3.3 “Angel Food Cake” and RStudio 2024.12.0+467. Operating System: Linux Mint 22.1 *Xia*. The different versions of the files *run_full_script.R* and *payoff rule.R* will be controlled at [https://github.com/Nestor-S-G/openscience]{.underline}
...
``` r
Error in windows(width = 8, height = 8) : no se pudo encontrar la función "windows"
```
