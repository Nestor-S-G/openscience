...
There is a crisis of reproducibility in multiple branches of science. Among other problems, difficulties in getting the same results out of the same data and code of the original studies have been found – that's what is called “Computational reproducibility”. Given its social relevance, this study has focused on the cases of computational reproducibility in neuroeconomics and behavioral finance. The result is that 58.6% of the studies could not be reproduced. Based on the experience acquired, a protocol for systematically assessing computational reproducibility is proposed.
...
| Aspect | Articles 1–3 | Article 4 | Remaining articles |
|------------------|------------------|------------------|------------------|
| Operating system | Linux Mint 22 *Wilma* | Linux Mint 22.1 *Xia* | Linux Mint 22.2 *Zara* |
| Error resolution support | ChatGPT-4o | ChatGPT-4o | ChatGPT-5.2 |
| Unavailability policy | Process suspended until available | Process suspended until available | Process suspended until available |
| R version | 4.3.3 “Angel Food Cake” | 4.3.3 “Angel Food Cake” | 4.5.2 “\[Not\] Part in a Rumble” |
| RStudio version | 2024.09.1+394 | 2024.12.0+467 | 2025.09.2 Build 418 |
...
The analysis of the field journal resulted in the following thematic categories: "data not shared", problems of *software incompatibility, related to the files to be analyzed*, and *code problems*, which were grouped into the supercategory *Errors*. The *Results* obtained are divided among all categories.

Coding:

| Error category                   | Number of errors (n = 32) |
|----------------------------------|---------------------------|
| Software incompatibility         | 4                         |
| Issues related to files analyzed | 5                         |
...
| Partially reproducible with modifications | 9                          |
| Reproducible                              | 1                          |
| Reproducible with modifications           | 1                          |
...
White, E. P., Baldridge, E., Brym, Z. T., Locey, K. J., McGlinn, D. J. & Supp, S. R. (2013). Nine simple ways to make it easier to (re)use your data. *Ideas in Ecology and Evolution, 6*(2), Article 2. [[https://ojs.library.queensu.ca/index.php/IEE/article/view/4608]{.underline}](https://ojs.library.queensu.ca/index.php/IEE/article/view/4608)
...
The rest of the code runs without errors. I proceed to compare the results I get with the results published. I find some mismatches, such as results of 'Table 2' (according to the code) which are actually published under 'Table 1' in the paper or some differences due to rounding in Table B6. The only real mismatch is found in Table B2, where I am unable to make sense of my results compared to the published ones.

**Article 6:** *Card or dice? An improved experimental approach to measure dishonesty*, by Hermann et al. (2025).

I access [the link](https://osf.io/zhnx2) provided in the paper. There I find a related project link where the data is shared. I download the .xlsx file. No computational environment, code or instructions are shared. So I try to deduce a way to analyze the data using RStudio.

When I analyze the data using the script *CardOrDice.R*, I find results that are coherent with the ones published in the paper and some small differences probably due to rounding or using different software to analyze the data.
...
I download the data and, again, I find a .xlsx and a .docx file - no computational environment or code to process the data. I write an R script to analyze it and find virtually the same results, with some variations (44.2% in the paper vs 44.6% in my results).
...
I run the *pilot* and get virtually the same results, with small differences such as F-value of 133.34 reported vs. 133.93. Similar results are found in most experiments but *Experiment 5*, where discrepancies of sample (292 reported vs 288, of which 277 valid) are found, which leads to discrepancies in the mean, SD, and n values.
...
When downloading the materials I realize some belong to a file type unknown to me: .boxs. However, they seem unnecessary to analyze the data in *Interplay_IS_CE_Data.xls* file. Again, no code or computational environment is provided, so I develop an R script to proceed with the analysis.

12/22/25

After checking different versions of my code, I achieve results that are coherent with the reported ones, with some differences (such as a b value = -0.24 reported vs -0.22 in my case).

**Article 12:** *Making a promise increases the moral cost of lying: Evidence from Norway and the United States*, by Ekström et al. (2025).

Shared material are partly in .do, partly in .R files. Again, since Stata is proprietary software, I have to write a code based upon the info in the paper, at least for part of the reproduction process.

First I run the *TableA3_code.R* and compare it with the materials provided in the appendix. They match the reported figures, except for:
...
Then I proceed to write a script to analyze the rest of the data. All data matches the reported up to the third decimal point.

**Article 13:** *Not all about the money: Service quality information improves consumer decision-making*, by Blijlevens et al. (2024).

I check the link provided in the paper, but all I find is a pdf file that contains no data.

**Article 14:** *Ovulatory shift, hormonal changes, and no effects on incentivized decision-making*, by Fišar et al. (2023).

Again, shared materials are for Stata, as well as files with extensions .ztq and .ztt.

I develop a code in R to analyze the .dta files and I get some results close to the original ones, but not the same. For instance in table 5 I get:

``` r
testosterone          -0.0109513  0.0132570  -0.826   0.4097
```

While the paper reports −0.0081 and 0.0109.

**Article 15:** *Pay all subjects or pay only some? An experiment on decision-making under risk and ambiguity*, by Aydogan et al. (2024).

This paper contains a readme.txt with the version of Stata used to analyze the data. Again, I have to write a script to make my own analysis. This yields essentially the same results that the reported ones in Aydogan et al. (2024),

``` r
print(table_6_dist)
# A tibble: 2 × 7
  treatment Amb_Aversion Amb_Neutral Amb_Seeking CR_Aversion CR_Neutral CR_Seeking
  <fct>            <dbl>       <dbl>       <dbl>       <dbl>      <dbl>      <dbl>
1 PPS               52.3        34.1        13.6        37.5       38.6       23.9
2 RPS               55.8        31.4        12.8        42.0       34.1       23.9
```

12/23/25

**Article 16:** *The arithmetic of outcome editing in financial and social domains*, by Barrafrem et al. (2021).

Two .dta files are shared, but neither code nor instructions. I develop a code for R and proceed to analyze the data. After that, I find figures that match the reported ones, as well as some small discrepancies:

``` r
5 fixed  numeracy                -0.0178    0.00708     -2.52  -0.0317    -0.00394
```

Such as the -0.0178, reported as -0.020 in Table 2.

12/26/25

**Article 17:** *The effects of task difficulty and presentation format on eye movements in risky choice*, by Zhang et al. (2024).

Authors have share data and scripts for Stata, but no computational environment or info of what version of Stata was used.

After developing a script for R, I realized some results are not reproducible with the materials shared. Among the reproducible ones, I get minor discrepancies due to rounding.

**Article 18:** *The impact of language on decision-making: Auction winners are less cursed in a foreign language*, by Fu et al. (2025).

Again, materials are shared for Stata. A README.txt file is included with the version of Stata (16+) required. After writing a code for R, all I get is some results that are close to the reported ones, but not the same:

``` R
======================================================
> cat("   SECTION 3.1: ROUND 1 DESCRIPTIVES (PDF PAGE 6)\n")
   SECTION 3.1: ROUND 1 DESCRIPTIVES (PDF PAGE 6)
> cat("========================================================\n")
========================================================
> cat("Native Mean Naive Dev (Paper: -4.76):  ", round(r1_stats$mean_naive_dev[1], 2), "\n")
Native Mean Naive Dev (Paper: -4.76):   -4.82 
> cat("Foreign Mean Naive Dev (Paper: -20.23): ", round(r1_stats$mean_naive_dev[2], 2), "\n")
Foreign Mean Naive Dev (Paper: -20.23):  -19.87 
> cat("Difference (Paper: -15.47):             ", round(r1_stats$mean_naive_dev[2] - r1_stats$mean_naive_dev[1], 2), "\n")
Difference (Paper: -15.47):              -15.05 
> cat("t-statistic (Paper: 4.37):              ", round(abs(t_test_r1$statistic), 2), "\n")
t-statistic (Paper: 4.37):               4.31 
> cat("p-value (Paper: < 0.001):               ", format.pval(t_test_r1$p.value), "\n")
p-value (Paper: < 0.001):                2.1498e-05 
```

**Article 19:** *Volatility shocks and investment behavior*, by Huber et al. (2021).
...
|------------------------------------|------------------------------------|
| Alem et al. (2023) | Partially reproducible with modifications |
| Aydogan et al. (2024) | Reproducible with modifications |
...
| Ekström et al. (2025) | Partially reproducible with modifications |
| Fareri et al. (2022) | Non-reproducible |
| Fišar et al. (2023) | Non-reproducible |
| Fred van Raaij et al. (2020) | Non-reproducible |
| Fu et al. (2025) | Non-reproducible |
...
| Payzan-LeNestour et al. (2024) | Partially reproducible with modifications |
...
| Zhang et al. (2024) | Partially reproducible with modifications |
